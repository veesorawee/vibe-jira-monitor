    ...
    const chartData = useMemo(() => {
        ...
        while (currentMonday <= maxDate) {
            ...
            if (stackBy === 'total') {
                ...
                uniqueBiCategories.forEach(cat => { dataPoint[cat] = 0; });
            }
        ...
        tasks.forEach(task => {
            ...
            if (weeklyBuckets.has(weekKey)) {
                if (stackBy === 'total') {
                    ...
                    if (uniqueBiCategories.includes(category)) {
        ...
        const colors = stackBy === 'total' ? { 'Total': '#8884d8' } : biCategoryColors;
        ...
        <div className={`fixed inset-0 z-50 transition-opacity ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            ...
            <div className={`fixed top-0 right-0 h-full bg-white w-full max-w-4xl shadow-xl transition-transform transform ease-in-out duration-300 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                 <div className="flex items-center justify-between p-4 border-b flex-shrink-0">
                     <h3 className="text-lg font-semibold truncate" title={labelName}>Task Summary for: {labelName}</h3>
                 ...
                 <div className="p-4 space-y-4 overflow-y-auto">
                    <div className="flex justify-center">
                        <div className="bg-gray-100 p-1 rounded-lg shadow-inner inline-flex items-center">
                            <button onClick={() => setStackBy('total')} className={`px-3 py-1 rounded-md transition-colors text-sm ${stackBy === 'total' ? 'bg-white text-blue-600 shadow' : 'text-gray-600 hover:bg-gray-200'}`}>Total</button>
